...
const fillAnswerLabel = document.getElementById("fillAnswerLabel");
const feedbackAudioRow = document.getElementById("feedbackAudioRow");
const feedbackAudioBtn = document.getElementById("feedbackAudioBtn");
...
function validListeningAnswers(card) {
  return [normalizedAnswer(card.speech || card.pt || "")].filter(Boolean);
}

function answerMatches(answer, validAnswers) {
  return validAnswers.includes(normalizedAnswer(answer));
}
...
function makeQuestion(type, cards) {
  ...
  let pool = cards;
  if (effectiveType === "fill" || effectiveType === "listening") {
  ...
  if (effectiveType === "listening") {
    return {
      type: "listening",
      card,
      instruction: "Escute e escreva o que você ouviu.",
      prompt: ""
    };
  }
  ...
  return {
    type: "choice",
    card,
    instruction: "Leia, escute e escolha o significado em inglês.",
    prompt: card.pt,
...
function startPractice(type) {
  ...
  if (type === "mixed") {
    ...
    practiceQueue = selected.map(card => {
      if ((type === "fill" || type === "listening") &&
          !fillEligibleCards([card]).length) {
        return makeQuestion(type, cards);
      }
...
function practiceLabel(type) {
  ...
  if (type === "fill") return "Escreva em português";
  if (type === "listening") return "Escute e escreva";
  return "Prática";
}
...
function renderPracticeQuestion() {
  ...
  const typedResponse = question.type === "fill" || question.type === "listening";
  fillForm.hidden = !typedResponse;

  if (question.type === "fill") {
    fillAnswerLabel.textContent = "Sua resposta em português";
  } else if (question.type === "listening") {
    fillAnswerLabel.textContent = "Escreva o que você ouviu";
  }

  // Áudio disponível antes da resposta em Escolha e Escute e escreva.
  // Em Escreva em português, aparece apenas no feedback.
  exerciseListenBtn.hidden = question.type === "fill";
...
function finishAnswer(correct, correctText) {
  ...
  feedbackAudioRow.hidden = practiceQuestion.type !== "fill";
...
function checkFillAnswer() {
  ...
  const isListening = practiceQuestion.type === "listening";

  const validAnswers = isListening
    ? validListeningAnswers(card)
    : validWrittenAnswers(card);

  const correct = answerMatches(fillAnswer.value, validAnswers);

  const displayAnswer = isListening
    ? (card.speech || card.pt)
    : card.pt;

  finishAnswer(correct, displayAnswer);
}
...
feedbackAudioBtn.addEventListener("click", speakFeedbackAnswer);
